...
ei_sample_plot <- ei_test %>% 
    bind_rows(ei_train) %>% 
    drop_na() %>% 
    group_by(sample) %>% 
    sample_n(size = 2000)

ggplot(ei_sample_plot, aes(x = zz_delt_vp, y = ei_score, 
    color = sample)) +
    theme_minimal() +
...
```{r}
library(colorspace)
ei_sample <- net_ei$ei_df %>% 
    group_by(ei_class_max) %>% 
    sample_n(size = 500)

ggplot(ei_sample, aes(x = ei_class_max, y = ei_score, 
    color = ei_score_var)) +
